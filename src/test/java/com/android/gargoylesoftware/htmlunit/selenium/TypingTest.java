 ...
 */
package com.android.gargoylesoftware.htmlunit.selenium;
...
public class TypingTest extends SeleniumTest {
    ...
    public void nonPrintableCharactersShouldWorkWithContentEditableOrDesignModeSet() {
        ...
        WebTestCase.assertEquals("Fishee!", element.getText());
    }
     ...
     */
    @Test
    @BrowserRunner.Alerts(DEFAULT = {"keydown (target) keyup (target) keyup (body)",
    ...
    public void canSafelyTypeOnElementThatIsRemovedFromTheDomOnKeyPress() {
        ...
        WebTestCase.assertEquals("", log.getAttribute("value"));

        input.sendKeys("b");
        WebTestCase.assertEquals(getExpectedAlerts()[0], getValueText(log).replace('\n', ' '));
        ...
        WebTestCase.assertEquals(getExpectedAlerts()[1], getValueText(log).replace('\n', ' '));
    }
