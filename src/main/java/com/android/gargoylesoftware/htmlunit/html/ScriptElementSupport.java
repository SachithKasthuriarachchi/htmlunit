...
public final class ScriptElementSupport {
        ...
        if (src != DomElement.ATTRIBUTE_NOT_DEFINED) {
            if (!src.startsWith(JavaScriptURLConnection.JAVASCRIPT_PREFIX)) {
                ...
                try {
                    ...
                    final HtmlPage.JavaScriptLoadResult result = page.loadExternalJavaScriptFile(src, charset);
    ...
    private static void executeInlineScriptIfNeeded(final DomElement element) {
        ...
        if (event != DomElement.ATTRIBUTE_NOT_DEFINED && forr != DomElement.ATTRIBUTE_NOT_DEFINED) {
            if (element.hasFeature(BrowserVersionFeatures.JS_SCRIPT_SUPPORTS_FOR_AND_EVENT_WINDOW) && "window".equals(forr)) {
