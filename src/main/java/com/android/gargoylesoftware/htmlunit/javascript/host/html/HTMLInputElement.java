...
import com.android.gargoylesoftware.htmlunit.html.InputElementFactory;
...
public class HTMLInputElement extends FormField {
    ...
    public String getType() {
        ...
        else if (!browserVersion.hasFeature(BrowserVersionFeatures.HTMLINPUT_TYPE_DATETIME_SUPPORTED)) {
            switch (type) {
                case "date":
                case "time":
                case "datetime-local":
        ...
        else if (browserVersion.hasFeature(BrowserVersionFeatures.HTMLINPUT_TYPE_MONTH_NOT_SUPPORTED)) {
            switch (type) {
                case "datetime-local":
                case "month":
                case "week":
        ...
        if ("color".equals(type) && browserVersion.hasFeature(BrowserVersionFeatures.HTMLINPUT_FILES_UNDEFINED)) {
            type = "text";
        }
        return type;
    }
    ...
    public Object getSelectionStart() {
        ...
        if (dom instanceof SelectableTextInput) {
            if ("number".equalsIgnoreCase(getType())
                    && getBrowserVersion().hasFeature(BrowserVersionFeatures.JS_INPUT_NUMBER_SELECTION_START_END_NULL)) {
                return null;
            }
    ...
    public void setAlign(final String align) {
        final boolean ignoreIfNoError = getBrowserVersion().hasFeature(BrowserVersionFeatures.JS_ALIGN_FOR_INPUT_IGNORES_VALUES);
    ...
    public void click() throws IOException {
        ...
        final Event event;
        if (getBrowserVersion().hasFeature(BrowserVersionFeatures.EVENT_ONCLICK_USES_POINTEREVENT)) {
