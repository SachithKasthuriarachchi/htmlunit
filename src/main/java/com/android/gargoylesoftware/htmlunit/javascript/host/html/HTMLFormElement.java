...
public class HTMLFormElement extends HTMLElement implements Function {
    ...
    public HTMLCollection getElements() {
        ...
        return new HTMLCollection(htmlForm, false) {
            ...
            @Override
            public AbstractList.EffectOnCache getEffectOnCache(final HtmlAttributeChangeEvent event) {
    ...
    public String getAction() {
        ...
        if (action != DomElement.ATTRIBUTE_NOT_DEFINED
                && action.length() == 0
                && getBrowserVersion().hasFeature(BrowserVersionFeatures.JS_FORM_ACTION_EXPANDURL_IGNORE_EMPTY)) {
            return action;
        }

        if (action == DomElement.ATTRIBUTE_NOT_DEFINED
                && !getBrowserVersion().hasFeature(BrowserVersionFeatures.JS_FORM_ACTION_EXPANDURL_NOT_DEFINED)) {
            return action;
        }
    ...
    public void submit() {
        ...
        if (StringUtils.startsWithIgnoreCase(action, JavaScriptURLConnection.JAVASCRIPT_PREFIX)) {
            ...
            final boolean forceDownload = webClient.getBrowserVersion().hasFeature(BrowserVersionFeatures.JS_FORM_SUBMIT_FORCES_DOWNLOAD);
