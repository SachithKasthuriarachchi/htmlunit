...
import com.android.gargoylesoftware.htmlunit.html.impl.SimpleRange;
...
import com.android.gargoylesoftware.htmlunit.util.EncodingSniffer;
...
import com.android.gargoylesoftware.htmlunit.html.HtmlSvg;
import com.android.gargoylesoftware.htmlunit.html.HtmlUnknownElement;
import com.android.gargoylesoftware.htmlunit.html.UnknownElementFactory;
...
import com.android.gargoylesoftware.htmlunit.javascript.host.NativeFunctionPrefixResolver;
...
public class Document extends Node {
    ...
    public void setDesignMode(final String mode) {
        ...
        final boolean inherit = browserVersion.hasFeature(BrowserVersionFeatures.JS_DOCUMENT_DESIGN_MODE_INHERIT);
        if (inherit) {
    ...
    public Object getApplets() {
        if (getBrowserVersion().hasFeature(BrowserVersionFeatures.HTMLDOCUMENT_APPLETS_NODELIST)) {
     ...
     */
    @JsxGetter({SupportedBrowser.CHROME, SupportedBrowser.IE, SupportedBrowser.EDGE})
    ...
    public Event createEvent(final String eventType) throws DOMException {
        ...
        if (clazz == null
                ...
                || "MouseWheelEvent".equals(eventType)
                    && getBrowserVersion().hasFeature(BrowserVersionFeatures.EVENT_TYPE_MOUSEWHEELEVENT)
                ...
                || "FocusEvent".equals(eventType)
                    && getBrowserVersion().hasFeature(BrowserVersionFeatures.EVENT_TYPE_FOCUSEVENT)
                || "SVGZoomEvent".equals(eventType)
                    && getBrowserVersion().hasFeature(BrowserVersionFeatures.EVENT_TYPE_SVGZOOMEVENT)
                || "WheelEvent".equals(eventType)
                    && getBrowserVersion().hasFeature(BrowserVersionFeatures.EVENT_TYPE_WHEELEVENT))) {
            clazz = SUPPORTED_VENDOR_EVENT_TYPE_MAP.get(eventType);

            if (PopStateEvent.class == clazz
                    && getBrowserVersion().hasFeature(BrowserVersionFeatures.EVENT_ONPOPSTATE_DOCUMENT_CREATE_NOT_SUPPORTED)) {
                clazz = null;
            }
        }
        if (clazz == null) {
    ...
    public Object createTreeWalker(final Node root, final double whatToShow, final Scriptable filter,
        ...
        final boolean filterFunctionOnly = getBrowserVersion().hasFeature(BrowserVersionFeatures.JS_TREEWALKER_FILTER_FUNCTION_ONLY);
    ...
    private static Date parseDateOrNow(final String stringDate) {
        final Date date = StringUtils.parseHttpDate(stringDate);
        if (date == null) {
            return new Date();
        }
        return date;
    ...
    public String getXmlEncoding() {
        ...
        if (encoding == null && getBrowserVersion().hasFeature(BrowserVersionFeatures.HTMLDOCUMENT_CHARSET_LOWERCASE)) {
            encoding = "";
        }
        return encoding;
    }
    ...
    public String getXmlVersion() {
        ...
        if (version == null && getBrowserVersion().hasFeature(BrowserVersionFeatures.HTMLDOCUMENT_CHARSET_LOWERCASE)) {
            version = "";
        }
        return version;
    }
     ...
     */
    @JsxGetter({SupportedBrowser.CHROME, SupportedBrowser.IE, SupportedBrowser.FF52})
     ...
     */
    @JsxSetter({SupportedBrowser.CHROME, SupportedBrowser.IE, SupportedBrowser.FF52})
