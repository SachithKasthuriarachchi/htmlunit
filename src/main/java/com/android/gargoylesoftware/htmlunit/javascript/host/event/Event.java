...
public class Event extends SimpleScriptable {
     ...
     */
    @JsxGetter({SupportedBrowser.IE, SupportedBrowser.CHROME, SupportedBrowser.EDGE})
     ...
     */
    @JsxGetter({SupportedBrowser.FF, SupportedBrowser.IE, SupportedBrowser.EDGE})
    ...
    public void initEvent(final String type, final boolean bubbles, final boolean cancelable) {
        ...
        bubbles_ = bubbles;
        cancelable_ = cancelable;
        if (TYPE_BEFORE_UNLOAD.equals(type) && getBrowserVersion().hasFeature(BrowserVersionFeatures.EVENT_FOCUS_FOCUS_IN_BLUR_OUT)) {
            try {
